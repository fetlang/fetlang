...
static bool isOnesPlace(const std::string& w) { return isOnesPlace(wordToInteger(w)); }
// number between 10-19 inclusive
static bool isTeen(FractionInt i) { return (i >= 10 && i < 20); }
static bool isTeen(const std::string& w) { return isTeen(wordToInteger(w)); }
...
static bool isTensPlace(const std::string& w) { return isTensPlace(wordToInteger(w)); }
...
static bool isTripleSeparator(const std::string& w) { return isTripleSeparator(wordToInteger(w)); }
...
// Convert a triple to a number (triple is list of words representing a number with a max of three
// [decimal] digits)
	...
	*/
	...
	for (const std::string& word : triple) {
		...
		if (word == "and") {
			...
			// Hundred can appear only once, after 1-9, if it is the second word (not including
			// and)o
			...
			// A ones place number must only appear before 100, after 'and', or after 'twenty',
			// 'thirty' etc or by itself
			if (hundred_can_appear || (previous_word == "and" || isTensPlace(previous_word))) {
				...
				throw FetlangException(
					"A ones-place number can only appear in a triple before 100, after 'and', or "
					"after a ten's place number");
			}
		} else if (isTeen(word)) {
			// A teen is similar, but it can't appear after a ten's place word
			// so, only by itself or after an 'and'(which could be after 100, or not; it doesn't
			// matter)
			if (hundred_can_appear || previous_word == "and") {
				...
				throw FetlangException(
					"A 'ten', 'eleven', 'twelve', or 'x-teen' can only appear by itself, or after "
					"'and' in triple");
			}
			...
			if (hundred_can_appear || previous_word == "and") {
				...
				throw FetlangException(
					"A ten's place number can only appear by itself, or after 'and', in triple");
			}
		}
...
// Convert any list representing an integer value to its actual value(as an integer fraction, or
// infinity)
	...
	for (const std::string& word : number) {
		if (word != "and" && isTripleSeparator(word)) {
			...
			if (separator_value >= previous_separator_value) {
				throw FetlangException(
					"Triple separator values must be in descending order with no repeats");
			}
