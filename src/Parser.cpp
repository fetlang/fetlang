...
static bool tokenIsObjectivePronoun(const Token& token) {
	return tokenIsPronoun(token) && manager.getPronoun(token.getValue()).isObjective();
}

static bool tokenIsReflexivePronoun(const Token& token) {
	return tokenIsPronoun(token) && manager.getPronoun(token.getValue()).isReflexive();
}
...
void Parser::transplacePronounToken(std::vector<Token>::iterator& token) {
	...
	if (!variables.hasLastOfGender(manager.getPronoun(token->getValue()).getGender())) {
	...
	Variable var = variables.getLastOfGender(manager.getPronoun(token->getValue()).getGender());
...
void Parser::transplaceSecondPronounToken(std::vector<Token>::iterator& token) {
	...
	// tokenward token with it
	Variable var =
		variables.getLastOfGenderExcludingLast(manager.getPronoun(token->getValue()).getGender());
	transplaceTokenWithVariable(token, var);
}
...
void Parser::transplaceIdentifierToken(std::vector<Token>::iterator& token, FetType other_type,
									   const Operator& op, bool isRightward, bool isReversed) {
		...
		if (other_type == UNKNOWN_TYPE) {
			if (isRightward) {
				throw TokenException(
					"Can't infer rightward operand without context - what are you, crazy? The "
					"leftward operand should have been guessed already",
					*token);
			}
			if (isReversed) {
			...
			actual_type =
				isActuallyRight ? op.inferRightByLeft(other_type) : op.inferLeftByRight(other_type);
		}
...
FetType Parser::getFetTypeOfOperand(const Token& token) {
	...
	if (tokenIsReflexivePronoun(token)) {
		return UNKNOWN_TYPE;
	}
	if (token.getCategory() == Token::IDENTIFIER_TOKEN && !variables.has(token.getValue())) {
		return UNKNOWN_TYPE;
	}

	// Literals we know very easily
	if (token.getCategory() == Token::CHAIN_LITERAL_TOKEN) {
		return CHAIN_TYPE;
	}
	if (token.getCategory() == Token::FRACTION_LITERAL_TOKEN) {
...
void Parser::parseOperation(const Grammar& grammar, std::vector<Token>::iterator left,
							const Operator& op, std::vector<Token>::iterator right,
	...
	if (!op.hasGrammar(grammar.getName())) {
		throw FetlangException(std::string("'" + op.getName() + "' operator cannot be used in ") +
	...
	if (tokenIsObjectivePronoun(*left)) {
		...
		if (!grammar.allowsLeftPronoun()) {
			throw TokenException(grammar.getName() + " grammar does not allow leftward pronouns",
								 *left);
		}
		...
		if (!grammar.allowsLeftIdentifier()) {
			throw TokenException(grammar.getName() + " grammar does not allow leftward identifiers",
								 *left);
		}
	...
	if (tokenIsObjectivePronoun(*right)) {
		...
		if (!grammar.allowsRightPronoun()) {
			throw TokenException(grammar.getName() + " grammar does not allow rightward pronouns",
								 *right);
		}
		...
		if (!grammar.allowsRightReflexivePronoun()) {
			throw TokenException(
				grammar.getName() + "grammar does not allow rightward reflexive pronouns", *right);
		}
		// Same old
		Variable& var = variables.getLastAccessed();
		var.setGender(manager.getPronoun(right->getValue()).getGender());
		...
		if (!grammar.allowsRightIdentifier()) {
			throw TokenException(
				grammar.getName() + " grammar does not allow rightward identifiers", *right);
		}
		// Replace with variable IF NECESSARY
		transplaceIdentifierToken(right, getFetTypeOfOperand(*left), op, true,
								  grammar.isReversed());
		...
		if (!grammar.allowsRightChainLiteral()) {
			throw TokenException(
				grammar.getName() + " grammar does not allow rightward chain literals", *right);
		}
		...
		if (!grammar.allowsRightFractionLiteral()) {
			throw TokenException(
				grammar.getName() + " grammar does not allow rightward fraction literals", *right);
		}
		/* No need to replace! :D */
	} else {
...
void Parser::formBranch(Node& node) {
	...
	case Token::KEYPHRASE_TOKEN:
		...
		case CONTROL_KEYPHRASE: {
			...
			if (token_iterator->getValue() == "bind") {
				...
				if (!nextTokensAreAvailable(4)) {
					throw TokenException(
						"`bind` expects to be followed by at "
						"least four tokens",
						*token_iterator);
				}
				...
				if (!(tokenIsIdentifier(*token_iterator) ||
					...
					throw TokenException(
						"`bind` expects to be followed by "
						"objective pronoun or identifier",
						*token_iterator);
				}
				...
				bind_grammar.addAllowedRightwardOperands(
					{Grammar::IDENTIFIER, Grammar::PRONOUN, Grammar::CHAIN_LITERAL});
			...
			} else if (token_iterator->getValue() == "if" ||
					   token_iterator->getValue() == "while" ||
				...
				if (!nextTokensAreAvailable(4)) {
					throw TokenException(
						"`" + controller + "` expects to be followed by at least four tokens",
						*token_iterator);
				}
				...
				if (token_iterator->getCategory() != Token::IDENTIFIER_TOKEN) {
					throw TokenException("Expected identifier as lho in comparison operation",
										 *token_iterator);
				}
				auto lho = token_iterator;
				token_iterator++;
				...
				if (!tokenIsComparisonOperator(*token_iterator)) {
					throw TokenException("Expected comparison operator in comparison operation",
										 *token_iterator);
				}
				auto comparison_operator =
					manager.getComparisonOperator(token_iterator->getValue());
				token_iterator++;
				...
				if (token_iterator->getCategory() != Token::IDENTIFIER_TOKEN &&
					...
					throw TokenException(
						"Expected identifier or pronoun as rho in comparison operation",
						*token_iterator);
				}
				auto rho = token_iterator;
				token_iterator++;
				...
				if (token_iterator != tokens.end() && token_iterator->getValue() == "otherwise" &&
					indent_level == getLineIndent(token_iterator->getLine())) {
					if (controller != "if") {
						throw TokenException("Can only use `otherwise` after an `if` statement",
											 *token_iterator);
					}
				...
				have_grammar.addAllowedLeftwardOperands({Grammar::PRONOUN, Grammar::IDENTIFIER});
				have_grammar.addAllowedRightwardOperands(
					{Grammar::PRONOUN, Grammar::IDENTIFIER, Grammar::REFLEXIVE_PRONOUN});
				...
				make_grammar.addAllowedLeftwardOperands({Grammar::PRONOUN, Grammar::IDENTIFIER});
		...
		case OPERATOR_KEYPHRASE: {
			...
			plain_grammar.addAllowedLeftwardOperands({Grammar::PRONOUN, Grammar::IDENTIFIER});
			plain_grammar.addAllowedRightwardOperands({Grammar::FRACTION_LITERAL});
			...
			// Check if there's a given RHO
			if (token_iterator != tokens.end() &&
				token_iterator->getCategory() == Token::FRACTION_LITERAL_TOKEN) {
				...
				if ((token_iterator + 1) != tokens.end()) {
					if (token_iterator->getValue() == "one") {
						if ((token_iterator + 1)->getValue() != "time") {
							throw TokenException(
								"Expected 'time' after fraction literal 'one' in plain grammar",
								*(token_iterator + 1));
						}
					} else if ((token_iterator + 1)->getValue() != "times") {
						throw TokenException("Expected 'times' after fraction literal here",
											 *(token_iterator + 1));
					}
				} else {
					throw TokenException(
						"Expected 'time' or 'times' after fraction literal but there are no more "
						"tokens left",
						*token_iterator);
				}
		...
		default:
			throw TokenException(
				"Parser isn't sure how to deal with this type of keyword at this point in time",
				*token_iterator);
		}
		break;
	default:
		throw TokenException(
			"Parser isn't sure how to deal with this type of token at the top level",
			*token_iterator);
	}
}
