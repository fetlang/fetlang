...
static void replaceText(std::string& str, const std::string& o, const std::string& n) {
...
std::string Transpiler::transpileBranch(Node& node) {
	...
	if (node.getToken().getCategory() != Token::KEYPHRASE_TOKEN) {
		throw NodeException(
			"Transpiler expected a keyphrase, but found `" + node.getToken().getValue() + "`",
			node);
	}
	...
	case CONTROL_KEYPHRASE: {
		...
		if (node.getToken().getValue() == "bind") {
			...
			if (node.getChild(0).getNumberOfChildren() != 2) {
				throw NodeException("Expected this `to` node to have exactly two children",
									node.getChild(0));
			}
			...
			if (rho.getCategory() == Token::IDENTIFIER_TOKEN) {
				...
				if (rho_type == STREAM_TYPE) {
					rho_code =
						rho_temp_chain;  //"temp_chain_for_stream_for_bind"+std::to_string(rand());
					pre_code += "append_stream_to_chain(&" + rho_code + ", " +
								variables.get(rho.getValue()).getCode() + ");\n";
					post_code += "chain_to_stream(" + rho_code + ", " +
								 variables.get(rho.getValue()).getCode() + ");\n";
		...
		} else if (node.getToken().getValue() == "while" || node.getToken().getValue() == "until" ||
			...
			if (node.getToken().getValue() != "otherwise") {
				...
				if (node.getChild(0).getToken().getCategory() != Token::KEYPHRASE_TOKEN ||
					node.getChild(0).getToken().getKeyphraseCategory() !=
						COMPARISON_OPERATOR_KEYPHRASE) {
					throw NodeException(
						"Expected this node to be a comparison"
						" operator",
						node.getChild(0));
				}
				if (node.getChild(0).getNumberOfChildren() != 2) {
					throw NodeException(
						"Expected this comparison operator node to"
						" have exactly two children",
						node.getChild(0));
				}

				ComparisonOperator op =
					manager.getComparisonOperator(node.getChild(0).getToken().getValue());
				Token lho = node.getChild(0).getChild(0).getToken();
				Token rho = node.getChild(0).getChild(1).getToken();
				...
				if (lho.getCategory() != Token::IDENTIFIER_TOKEN) {
					throw TokenException(
						"Expected the LHO in a comparison operation to be an identifier token",
						lho);
				}
				if (rho.getCategory() != Token::IDENTIFIER_TOKEN) {
					throw TokenException(
						"Expected the RHO in a comparison operation to be an identifier token",
						rho);
				}
	...
	case OPERATOR_KEYPHRASE: {
		...
		if (node.getNumberOfChildren() != 2) {
			throw NodeException("Expected this node to have 2 children, but has " +
									std::to_string(node.getNumberOfChildren()),
								node);
		}
		...
		if (rho.getCategory() == Token::IDENTIFIER_TOKEN) {
			...
			throw TokenException("Cannot perform an operation on a right-hand operand of this type",
								 rho);
		}
		...
		if (rho.getCategory() == Token::IDENTIFIER_TOKEN) {
			if (rho_type == STREAM_TYPE && !op.hasCodeForExactly(lho_type, STREAM_TYPE)) {
				...
				rho_code = rho_temp_chain;
				pre_code +=
					"/* Stream chain(rho) initialization */\n"
					"append_stream_to_chain(&" +
					rho_code + ", " + variables.get(rho.getValue()).getCode() + ");\n\n";
				post_code +=
					"/* Stream chain(rho) deconstruction */\n"
					"clear_chain(&" +
					rho_code + ");\n\n";
			} else {
				rho_code = variables.get(rho.getValue()).getCode();
			...
			pre_code +=
				...
				"append_cstr_to_chain(&" +
				rho_code + ", " + QuoteUtil::requote(rho.getValue()) + ");\n\n";
			post_code +=
		...
		if (!op.hasCodeFor(lho_type, rho_type)) {
			throw NodeException(
				"Operator can't work with operands of these "
				"types(" +
					stringifyFetType(lho_type) + "/" + stringifyFetType(rho_type) + ")",
				node);
		}
	...
	default:
		throw NodeException(
			"Transpiler doesn't know what to do with this node at this point in time", node);
	}
...
std::string Transpiler::transpile() {
	...
	for (const Fetish& fetish : manager.getFetishes()) {
		for (const std::string& header_path : fetish.getIncludes()) {
			code +=
				"#include " + QuoteUtil::quote(fetish.getName() + "/include/" + header_path) + "\n";
		}
	}

	// Head code
	code += std::string("int main(int argc, char* argv[]){\n") + "Chain " + lho_temp_chain +
			";init_chain(&" + lho_temp_chain +
			");\n"
			"Chain " +
			rho_temp_chain + ";init_chain(&" + rho_temp_chain + ");\n";
